...
scaling_factor = 0.5
plt.scatter(df.index, df['Engine (cc)'], s=df['Engine (cc)'] * scaling_factor, color=df['Color'], alpha=0.6)
...
plt.bar(avg_engine_size.index, avg_engine_size.values, color=[colors[cat] for cat in avg_engine_size.index])
plt.xlabel('Category')
